...
<div class="row">
    <div class="col-md-12">
        <h3 class="page-title">{{ $langLbl['World Of Professional'] }} {{ $langLbl['Management'] }}</h3>
...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-plus"></i> {{ $langLbl['Edit'] }} {{ $langLbl['World Of Professional'] }}
        </div>
    </div>
    ...
    <?php
        ...
        $img_key = "image";
    ?>
        <form class="form-horizontal form-bordered form-row-stripped" role="form" method="post" action="{{ URL::route('admin.worldofprofession.store') }}" enctype="multipart/form-data">
            <input type="hidden" name="wof_id" value="{{ $wof->id }}">
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key3, $value3) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key3 }}" value="{{ $wof->post_titleen }}" />
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key4, $value4) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key4 }}" value="{{ $wof->post_titleit }}"  />
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key5, $value5) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key5 }}" value="{{ $wof->post_titlees }}"  />
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key, $value) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key }}" rows="10">{{ $wof->post_contenten }}</textarea>
                </div>
            </div>
             <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key1, $value1) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key1 }}" rows="10">{{ $wof->post_contentit }}</textarea>
                </div>
            </div>
             <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key2, $value2) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key2 }}" rows="10">{{ $wof->post_contentes }}</textarea>
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="fileinput fileinput-new" data-provides="fileinput">
                <div class="fileinput-new thumbnail" style="width: 120px; height: 120px;">
                    <img src="{{ HTTP_HOWITWORKS_PATH.$wof->image }} " alt=""/>
                </div>
            ...
            <div class="form-actions fluid">
                <div class="col-sm-12 text-center">
                    ...
                    <a href="{{ URL::route('admin.worldofprofession') }}" class="btn btn-primary">
