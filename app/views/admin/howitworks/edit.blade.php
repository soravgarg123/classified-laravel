...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption">
            <i class="fa fa-pencil-square-o"></i> {{ $langLbl['Edit'] }} {{ $langLbl['howitworks'] }}
        </div>
    </div>
        ...
        <form class="form-horizontal form-bordered form-row-stripped" role="form" method="post" action="{{ URL::route('admin.howitworks.store') }}" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <div class="col-sm-9">
                    <select name="type" class="form-control">
                        ...
                        <option value="Professional" <?php if($howitworks->type == "Professional") echo "selected"; ?> >Professional</option>
                        <option value="User" <?php if($howitworks->type == "User") echo "selected"; ?> >User</option>
                    </select>
                </div>
            </div>
            <input type="hidden" name="howitworks_id" value="{{ $howitworks->id }}"/>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key3, $value3) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key3 }}" value="{{ $howitworks->titleen }}" />
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key4, $value4) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key4 }}" value="{{ $howitworks->titleit }}"/>
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key5, $value5) }}</label>
                <div class="col-sm-9">
                    <input type="text" required class="form-control" name="{{ $key5 }}" value="{{ $howitworks->titlees }}" />
                </div>
            </div>
            <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key, $value) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key }}" rows="10">{{ $howitworks->contenten }}</textarea>
                </div>
            </div>
             <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key1, $value1) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key1 }}" rows="10">{{ $howitworks->contentit }}</textarea>
                </div>
            </div>
             <div class="form-group">
                <label class="col-sm-3 control-label">{{ Form::label($key2, $value2) }}</label>
                <div class="col-sm-9">
                    <textarea required class="form-control" name="{{ $key2 }}" rows="10">{{ $howitworks->contentes }}</textarea>
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="fileinput fileinput-new" data-provides="fileinput">
                <div class="fileinput-new thumbnail" style="width: 120px; height: 120px;">
                    <img src="{{ HTTP_HOWITWORKS_PATH.$howitworks->image }} " alt=""/>
                </div>
