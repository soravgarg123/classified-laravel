...
class PostCategoryController extends \BaseController {
    ...
    public function index() {
        $param['categories'] = PostCategoryModel::paginate(PAGINATION_SIZE);
        $param['pageNo'] = 14;
        if ($alert = Session::get('alert')) {
            $param['alert'] = $alert;
        }

        return View::make('admin.postcategory.index')->with('siteLanguage', $this->siteLanguage)->with($param)->with('langLbl', $this->languageLabels)
    ...
    public function create() {
        ...
        return View::make('admin.postcategory.create')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels)
                        ->with('currentLanguage', $this->currentLanguage);
    }

    public function edit($id) {
        $param['pageNo'] = 14;
        $param['category'] = PostCategoryModel::find($id);
        ...
        return View::make('admin.postcategory.edit')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels)
                        ->with('currentLanguage', $this->currentLanguage);
    }
    ...
    public function store() {
        foreach ($this->siteLanguage as $value) {
                ...
                $checkCat = DB::table('ds_postcategory')->where($nameCtrl, $inputval);
                if(!empty($checkCat)){
                    $alert['msg'] = $this->languageLabels['Category Already Added'];
                    $alert['type'] = 'danger';
                    return Redirect::route('admin.postcategory')->with('alert', $alert);
                }
            }
        $rules = ['name' => 'required'];
    ...
    public function delete($id) {
        try {
            PostCategoryModel::find($id)->delete();
