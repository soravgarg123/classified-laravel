...
class PclassController extends \BaseController {
    ...
    public function index() {
        $param['classes'] = PclassModel::paginate(PAGINATION_SIZE);
        $param['pageNo'] = 18;
        if ($alert = Session::get('alert')) {
            $param['alert'] = $alert;
        }

        return View::make('admin.class.index')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels);
    }
    ...
    public function create() {
        ...
        return View::make('admin.class.create')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels);
    }

    public function edit($id) {
        $param['pageNo'] = 18;
        $param['class'] = PclassModel::find($id);
        ...
        return View::make('admin.class.edit')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels);
    }
    ...
    public function store() {
        foreach ($this->siteLanguage as $value) {
                ...
                $checkCat = DB::table('ds_class')->where($nameCtrl, $inputval);
                if(!empty($checkCat)){
                    $alert['msg'] = $this->languageLabels['Category Already Added'];
                    $alert['type'] = 'danger';
                    return Redirect::route('admin.class')->with('alert', $alert);
                }
            }
        $rules = ['name' => 'required'];
        ...
        if ($validator->fails()) {
            ...
            if (Input::has('class_id')) {
                ...
                $class = PclassModel::find($id);
            } else {
                $class = new PclassModel;
            }
            foreach ($this->siteLanguage as $value) {
                $nameCtrl = 'name' . $value;
                $descCtrl = 'description' . $value;
                $class->$nameCtrl = Input::get($nameCtrl);
                $class->$descCtrl = Input::get($descCtrl);
            }
            $class->icon = Input::get('icon');
            $class->save();
