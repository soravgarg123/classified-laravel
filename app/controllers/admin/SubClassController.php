...
class SubClassController extends \BaseController {
    ...
    public function create($id) {
        ...
        return View::make('admin.class.sub.create')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels)
                        ->with('currentLanguage', $this->currentLanguage);
    }

    public function edit($id) {
        $param['pageNo'] = 5;
        $param['subClass'] = SubClassModel::find($id);
        ...
        return View::make('admin.class.sub.edit')->with($param)->with('siteLanguage', $this->siteLanguage)->with('langLbl', $this->languageLabels)
                        ->with('currentLanguage', $this->currentLanguage);
    }
    ...
    public function store() {
        foreach ($this->siteLanguage as $value) {
                $nameCtrl = 'name' . $value;
                $inputval = Input::get($nameCtrl);
                $checkSubCat = DB::table('ds_sub_class')->where($nameCtrl, $inputval);
                if(!empty($checkSubCat)){
                    $alert['msg'] = $this->languageLabels['Sub-category Already Added'];
                    $alert['type'] = 'danger';
                    return Redirect::route('admin.class.edit', Input::get('class_id'))->with('alert', $alert);
                }
            }
        $rules = ['name' => 'required'];
        ...
        if ($validator->fails()) {
            ...
            if (Input::has('sub_class_id')) {
                $id = Input::get('sub_class_id');
                $subClass = SubClassModel::find($id);
            } else {
                $subClass = new SubClassModel;
            }
            $subClass->class_id = Input::get('class_id');
            //$subClass->name = Input::get('name');
            $subClass->icon = Input::get('icon');
            //$subClass->description = Input::get('description');
            foreach ($this->siteLanguage as $value) {
                $nameCtrl = 'name' . $value;
                $descCtrl = 'description' . $value;
                $subClass->$nameCtrl = Input::get($nameCtrl);
                $subClass->$descCtrl = Input::get($descCtrl);
            }
            $subClass->save();

            $alert['msg'] = $this->languageLabels['Sub category has been saved successfully'];
            $alert['type'] = 'success';
    ...
    public function delete($id) {
        ...
        try {
            ...
            $alert['msg'] = $this->languageLabels['Sub category has been deleted successfully'];
            $alert['type'] = 'success';
        } catch (\Exception $ex) {
            $alert['msg'] = $this->languageLabels['This sub category has been already used'];
            $alert['type'] = 'danger';
        }

        return Redirect::route('admin.class.edit', $classId)->with('alert', $alert);
    }
