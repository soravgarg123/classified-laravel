...
+function ($) { "use strict";
  ...
  Fileupload.prototype.change = function(e) {
    ...
    var calculatedSize = file.size/(1024*1024); // in MB
    var split_extension = fileName.split(".");
    var ext = [ "jpg", "gif","png","jpeg" ];   
    if(jQuery.inArray(split_extension[1].toLowerCase(), ext ) == -1)
        {
          $(this).val(file.value = null);
          bootbox.alert("You Can Upload Only .jpg, gif, png, jpeg files !");
          return false;
        }
    if(calculatedSize > 2)
        {
            $(this).val(file.value = null);
            bootbox.alert("File size should be less than 2 MB ! ");
            return false;
        }
